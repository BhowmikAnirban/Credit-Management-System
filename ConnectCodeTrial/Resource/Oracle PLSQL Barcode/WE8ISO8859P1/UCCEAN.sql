...
CREATE OR REPLACE PACKAGE BODY CONNECTCODE_UCCEAN AS  
 ...
 FUNCTION replace234(data IN VARCHAR2, fncvalue IN NUMBER) RETURN VARCHAR2 IS
   ...
   barcodecharx     VARCHAR2(1);
   barcodechary     VARCHAR2(1);
 ...
 BEGIN
  ...
  IF numset=0 THEN
     fncstring:=fncstring||CHR(fncvalue);
...
END generateCheckDigitABUCCEAN;
 ...
 FUNCTION generateCheckDigitCUCCEAN(data IN VARCHAR2) RETURN VARCHAR2 IS
  ...
  weight NUMBER:=1;
  skip NUMBER:=0;
  barcodechar1 VARCHAR2(1):=CHR(0);
  barcodechar2 VARCHAR2(1):=CHR(0);
  CtoB NUMBER:= 100;
  BtoC NUMBER:= 99;
 BEGIN
 ...
 FUNCTION ENCODE (data IN VARCHAR2) RETURN VARCHAR2 IS
   ...
   x NUMBER:= 1;
   skip NUMBER:=0;
   barcodechar1 VARCHAR(1):=CHR(0);
   barcodechar2 VARCHAR(1):=CHR(0);
   CtoB VARCHAR2(1):=CHR(232);
   BtoC VARCHAR2(1):=CHR(231);
   numvalue NUMBER :=0;
	...
	IF skip=0 THEN
		...
		IF ASCII(barcodechar1)>=48 AND ASCII(barcodechar1)<=57 AND ASCII(barcodechar2)>=48 AND ASCII(barcodechar2)<=57 THEN
			...
			ELSE 
				numvalue := -1;
			END IF;
			returndata:=returndata || CHR(numvalue);
			x:=x+2;	
     ...
     returndata := CHR(237) || returndata || CHR(238);
