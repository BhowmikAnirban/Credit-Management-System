import Code39ASCII = net.connectcode.Code39ASCII;

window.onload = () => {
           /*
    var elementBarcode = document.getElementById("barcodeData");
    var barcode: Code39ASCII = new Code39ASCII(elementBarcode.innerHTML,false);
    ...
    */
    ...
    for (var x: number = 0; x < elementBarcode.length; x++) {
        var barcode: Code39ASCII = new Code39ASCII((<HTMLElement>elementBarcode[x]).innerHTML);
        var result: string = barcode.encode();
        var hrText: string = barcode.getHRText();
